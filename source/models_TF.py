...
class UNet:
    ...
    def create(self):
        ...
        self.X = tf.layers.conv2d(inputs=self.X, filters=1, kernel_size=(1, 1), padding='SAME', activation='relu')
        self.output = self.X


def conv_block(X, kernel_size, num_filters):
    X = tf.layers.conv2d(inputs=X, filters=num_filters, kernel_size=kernel_size, padding='SAME', activation='relu')
